...
// Menampilkan hasil ucapan dan waktu
console.log(ucapanWaktu);
console.log('WIB:', wib);
console.log('WITA:', wita);
console.log('WIT:', wit);
console.log('Sore/Malam:', salam2);
//Status
if (!Raol404.public) {
...
case "menu": case "help": {
    ...
    const newsletterInfo = {
        newsletterName: "Xena", // Nama saluran newsletter
        newsletterJid: "120363378800202820@newsletter", // JID newsletter
        thumbnailUrl: 'https://img5.pixhost.to/images/3085/569847007_skyzopedia.jpg', // Thumbnail saluran
    ...
    await Raol404.sendMessage(m.chat, {
        ...
        contextInfo: {
            ...
            externalAdReply: {
                showAdAttribution: true,
                title: global.botTitle,
                body: global.botDescription,
...
- Cooldown berlaku untuk beberapa fitur`.trim(),
 ...
 case 'tiktokdownload': {
    ...
    const outputVideoPath = path.join(tempDir, 'tiktok_video.mp4');

    if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir);

    try {
        // Check if URL is provided
        if (!text) return reply(`Please provide the TikTok URL. Example: ${prefix + command} https://vm.tiktok.com/ZMkc7jK1m/`);
        
        // Fetch video from the provided TikTok URL
        const response = await axios.get(`https://nekoapi.web.id/download/tiktok-download?url=${videoUrl}`);
        
        if (!response.data || !response.data.data || !response.data.data.video) {
            return reply('Sorry, unable to fetch the video from the provided URL.');
        }

        // Get video download link
        const videoDownloadUrl = response.data.data.video;
        ...
        writer.on('finish', async () => {
            // Send video once it's downloaded
            await Raol404.sendVideo(m.chat, { url: outputVideoPath }, { caption: 'Here is your TikTok video!' });
